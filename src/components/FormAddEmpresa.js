...
const FormAddEmpresa = ({ InfoPageEmpresa = true }) => {
    ...
    try {
      ...
      (async function loadInputsDepandLoc() {
        ...
        if (departamentosAll?.ok) {
          ...
          if (dptoEmpresaActual != null) {
            setDpto(dptoEmpresaActual?.id)
          }
          if (departamentosAll.dptoUser || dptoEmpresaActual != null) {
            const localidad = await loadLocalidades(dptoEmpresaActual?.id);
            if (localidad.ok) {
  ...
  const [form, setForm] = useForm({
    nombre_fantasia: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.nombre_fantasia
        : detalleEmpresaActual?.[0]?.nombre_fantasia
          ? detalleEmpresaActual?.[0]?.nombre_fantasia
          : detalleEmpresaActual?.nombre_fantasia,
    telefono: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.telefono
        : detalleEmpresaActual?.[0]?.telefono
          ? detalleEmpresaActual?.[0]?.telefono
          : detalleEmpresaActual?.telefono,
    nro_referencia: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.nro_referencia
        : detalleEmpresaActual?.[0]?.nro_referencia
          ? detalleEmpresaActual?.[0]?.nro_referencia
          : detalleEmpresaActual?.nro_referencia,
    celular: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.celular
        : detalleEmpresaActual?.[0]?.celular
          ? detalleEmpresaActual?.[0]?.celular
          : detalleEmpresaActual?.celular,
    direccion: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.direccion
        : detalleEmpresaActual?.[0]?.direccion
          ? detalleEmpresaActual?.[0]?.direccion
          : detalleEmpresaActual?.direccion,
    rut: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.rut
        : detalleEmpresaActual?.[0]?.rut
          ? detalleEmpresaActual?.[0]?.rut
          : detalleEmpresaActual?.rut,
    email: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.email
        : detalleEmpresaActual?.[0]?.email
          ? detalleEmpresaActual?.[0]?.email
          : detalleEmpresaActual?.email,
    razon_social: InfoPageEmpresa
      ? ""
      : state.rol.id == 3
        ? empresaAdmin.razon_social
        : detalleEmpresaActual?.[0]?.razon_social
          ? detalleEmpresaActual?.[0]?.razon_social
          : detalleEmpresaActual?.razon_social,
    ...
    localaidad: detalleEmpresaActual?.[0]?.localidad?.id
  });
  console.log(form)
